...
> **Актуально на:** 07.10.2025  
> Поддерживаемые регионы: **JP / SEA / RU**  
...
2. ▶️ В папке `installer` кликните правой кнопкой на `installer.ps1` → **Запустить с помощью PowerShell**.  
...
2. После первого запуска в `settings` создаются JSON-файлы с конфигурацией каждого окна.  
3. Настройте их вручную или через GUI (шестерня слева снизу).
...
В комплекте 5 базовых профилей:
