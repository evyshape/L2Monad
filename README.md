...
```json
{
    ...
    "BUY_LOOT_TOWN": true, // выкупать ли шмотки пока бегаем в городе
    "BUY_LOOT_RIP": true, // выкупать ли шмотки после смерти
    ...
    "PARTY_DUNGEON_HARD": 1, // Сложность пати данжа, от 1 до 4
    "DONATE_SHOP_PAGES": "1|2", // Страницы донат шопа для выкупа
    "ALLIANCE_BUTTON": 2, // Кнопка для доната в альянс, 0 = не донатим, 1 = 1.5кк, 2 = 6кк
    ...
    "NEED_CLAIM_CLAN": true, // При сборе наград нужен ли донат в клан
    "NEED_CLAIM_ALI": true, // При сборе наград нужен ли донат в альянс
    "NEED_CLAIM_BATTLE_PASS": true, // При сборе наград нужен ли батл пасс
    "NEED_CLAIM_DONATE_SHOP": true, // При сборе наград нужен ли выкуп донат-шопа
    "NEED_SHOP_AFTER_RIP": true, // Идти к бакалейщику после смерти
    "NEED_SHOP_AFTER_PVP_EVADE": true, // Идти к бакалейщику после пвп доджа
    "NEED_BACK_TO_SPOT_PARTY_DUNGEON": true, // После пати данжа возвращать на спот
    "FAST_DODGE": true, // При лоу-хп додже улетать сразу, без ожидания падения хп
    "PVP_ANSWER_CHECK_HP_ITERATIONS": 3, // Итераций проверок хп после пропадания пвп триггера в PVP_ANSWER
    "DELAY_PVP_ANSWER": 50, // Таймаут пвп-ответа перед возвратом на спот
    "MIN_SLEEP_AFTER_RIP": 150, // Мин. секунды сна после смерти в pvp-доджере
    "MAX_SLEEP_AFTER_RIP": 300, // Макс. секунды сна после смерти
    "MIN_LOW_HP_DODGE_SLEEP": 60, // Мин. секунды сна после лоу-хп доджа
    "MAX_LOW_HP_DODGE_SLEEP": 300, // Макс. секунды сна после лоу-хп доджа
    "MIN_PVP_DODGE_SLEEP": 120, // Мин. секунды сна после пвп доджа
    "MAX_PVP_DODGE_SLEEP": 240, // Макс. секунды сна после пвп доджа
    "AUTOHUNT_BEFORE_TP": true // Включать автобой до тп на спот (false = после)
}
```
...
- **Расстановка окон** -- диалог расстановки окон сеткой (узкий/широкий режим, автозаполнение с сохранением ближайшей позиции, пул + блок кнопок, режимы 2×2 / 3×3 / 4×3 и т.д.).
...
В комплекте 7 базовых профилей:
...
7. **Scheduler** ⏰— телепортируется в город - открывает меню - включает игровое расписание - ждёт 30 сек - возвращает в
 энерго - завершается.
---
...
- Убедись, что запускаешь installer.exe от имени администратора
...
- Основные пороги/задержки лежат в `bot/delays.py` (внутри обозначено что за что отвечает).
- Всё остальное настраивается **per-window** в JSON-конфиге окна (либо через GUI-шестерню).
